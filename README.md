...
一句话说明：**一个把「原始语料」变成「可直接用于训练的合格数据」的评估与转换流水线**——用两个模型（本地 Ollama + 云端 DeepSeek）对每条语料在 4 个维度上迭代打分 3 轮，按阈值过滤，最终输出 Alpaca / ShareGPT 标准训练格式。
...
│  4 维度 × 2 模型 × 3 轮（每轮发回上一轮意见修正）  │
└─────────────────────────────────────────────────┘
        │
        ▼
   评估报告 JSON（meta / rounds / final）
        │
        ├──────────────────────────────┐
        ▼                              ▼
┌──────────────────────┐      ┌──────────────────────┐
│  convert.py          │      │  visualize.py        │
│  过滤 + 转训练格式    │      │  统计可视化（print）  │
│  (overall>=阈值)      │      │  (均分/分布/一致性)   │
└──────────────────────┘      └──────────────────────┘
        │                              │
        ▼                              ▼
  Alpaca / ShareGPT JSONL         ASCII 统计报表
```

- **eval_loop.py**：核心评分器。读入一条语料，两个模型分别对 4 个维度打分；第 2、3 轮把上一轮双方「分数 + 理由」作为修正上下文发回，促使模型收敛。输出一份完整 JSON 报告。
- **convert.py**：过滤与格式转换。读取评估报告，仅保留 `final.overall_score >= 阈值` 的语料，把 `meta.text` 转成 Alpaca 或 ShareGPT 格式，输出 JSONL。
- **visualize.py**：统计分析。聚合一份或多份报告，输出合格率、各维度平均分、分数分布、模型打分一致性，并用 ASCII 柱状图展示。
...
编辑 `eval_loop.py` 顶部配置区，把 `DEEPSEEK_API_KEY` 替换为你的真实 Key：

```python
DEEPSEEK_API_KEY = "YOUR_DEEPSEEK_KEY"  # TODO: 替换为真实 Key
```

其他可改配置（同样在 `eval_loop.py` 顶部）：`OLLAMA_URL`、`OLLAMA_MODEL`、`DEEPSEEK_URL`、`DEEPSEEK_MODEL`、`TIMEOUT`、`MAX_RETRY` 等。

---

## 5. 脚本使用说明

### 5.1 eval_loop.py —— 多模型迭代打分
...
1. 在 `judge_prompts.jsonl` 中新增/修改一条提示词（设置新的 `name`）。
...
A：`eval_loop.py` 顶部的 `DEEPSEEK_API_KEY` 仍是占位符或 Key 错误。请填入真实的 DeepSeek API Key（在 DeepSeek 开放平台获取），并确认账户有可用额度。
...
**Q5：提示 `ModuleNotFoundError: No module named 'requests'`？**
...
- [FastChat 官方仓库](https://github.com/lm-sys/FastChat)
- [FastChat MT-Bench / LLM Judge 文档](https://github.com/lm-sys/FastChat/tree/main/fastchat/llm_judge)

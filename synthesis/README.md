...
├── test_llm.py        # 长调用进度提示回归测试（python test_llm.py，不联网）
...
不会把日志刷烂）；调用失败或 `Ctrl-C` 时打印「中断」，**异常原样抛出**，
上层的错误日志不会因此失真。实现是 `llm.py` 的 `long_call()`，
放在这里是因为它是所有 API 调用的唯一收口 —— 两个脚本自动都有。
...
- **数据集的「有效性」尚无实验支撑**：需求验证方案里的「精选 20% ≥ 全量 95%」
  还没跑。数据卡第 6 条已如实标注。
- ~~长等待时没有进度反馈~~ —— 已实现，见第 1 节的「长调用进度提示」。
